...
! Hamid
!> -----------------------------------
        real(dl), private :: xnew(Nz)
        real(dl), private :: xtemp(Nz)
!< -----------------------------------
...
! Hamid
        !> -----------put a write statement to output the ionized fraction
        !             and read the data from the input
        open(unit=10, file='x_e.dat')
        write(10, '(a12,4a24)') "z", "x_e", "x0", "x_e/x0 - 1", "x_e - x0"
        ! ------------------------------------------------ <
...
!            x = x0
! Hamid
            !> --------------- creating a distortion function for the reionized Helium fraction
            x = x0
            !Calc%xnew(i) = x0
            Calc%xtemp(i) = x0
            ...
            if (di_in > 0) then
                if (i > di_in) then
                    Calc%xnew(i) = Calc%xtemp(i - di_in)
                    Calc%xrec(i) = Calc%xnew(i)
...
! Hamid > ----------------- implement the distortion function
        do i = 1,Nz
            Calc%xrec(i) = Calc%xrec(i) * ( 1._dl &
                         + distort*dexp(-(Calc%zrec(i)-z_ref)**2/(2._dl*sig_w**2)) )
            x = Calc%xrec(i)

            write(10, '(5g24.16)') Calc%zrec(i), x, Calc%xtemp(i), x/Calc%xtemp(i) - 1._dl, x - Calc%xtemp(i)

        end do
        write(10,*) "di_in =", di_in
        close(10)
! --------------------------- <
